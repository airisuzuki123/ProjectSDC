...
> 更新时间：2026-07-27
>
> 当前阶段：Phase 38 已完成，Phase 39 尚未启动。
>
> 本次交接目标：将仓库推送到 `origin/master` 后，后续开发从商店、仓库、配装和破产应急手枪开始，不重复实现上一阶段内容。

## 1. 项目定位

ProjectSDC 是基于纯 HTML、CSS 和 JavaScript 的搜打撤 Web 游戏原型，业务代码位于 `third_party/search-strike-extract`。

Phase 1-38 已完成单局 Demo、房间战斗、随机挑战包、局内构筑、占格背包、脚本化挑战基线、五关数据、房间预算、进度存档迁移、五关入口、顺序解锁展示和遗物永久货币结算。下一版本目标是接入商店、仓库、配装和破产应急手枪；不进入装备强化或角色长期养成。
...
2. Phase 39：商店、仓库、配装和破产应急手枪。（下一阶段）
3. Phase 40：跨关卡资产返还、再次带入和失败销毁。
...
- `js/main.js` 的 Demo 结算已接入 Phase 38 遗物货币，但仍未接入商店、仓库配装或跨关卡资产返还，需要在 Phase 39-40 分阶段接回，不能一次性解除所有分支。
...
最近验证结果：两条命令均为 `104 passed, 0 failed`。
...
4. 只启动 Phase 39：接入商店、仓库、配装和破产应急手枪，不提前接入跨关卡资产返还、再次带入或失败销毁。
5. Phase 39 完成后更新计划、交接和测试，再提交推送。

## 16. 建议的新对话开场指令

```text
请读取 D:\codex\ProjectSDC\AGENTS.md、docs\development-handoff.md 和 docs\development-plan.md，检查 Git 状态并运行完整 smoke 与 Phase 35 脚本基线。确认 Phase 38 已推送且工作区干净后，只启动 Phase 39：接入商店、仓库、配装和破产应急手枪；不要提前开发跨关卡资产返还、再次带入或失败销毁。
```

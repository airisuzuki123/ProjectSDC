...
> 更新时间：2026-07-27
>
> 当前阶段：Phase 35 已完成，Phase 36 尚未启动。
>
> 本次交接目标：将仓库推送到 `origin/master` 后，后续开发从五关数据与存档基础开始，不重复实现上一阶段内容。

## 1. 项目定位

ProjectSDC 是基于纯 HTML、CSS 和 JavaScript 的搜打撤 Web 游戏原型，业务代码位于 `third_party/search-strike-extract`。

Phase 1-35 已完成单局 Demo、房间战斗、随机挑战包、局内构筑、占格背包和脚本化挑战基线。下一版本目标是构建五关顺序挑战与轻量局外资产循环，包括遗物货币、商店、仓库、配装和失败损失；不进入装备强化或角色长期养成。
...
1. Phase 36：五关数据、常规挑战房预算、进度存档和旧存档迁移。
2. Phase 37：五关入口、锁定状态、普通撤离与完美撤离解锁链路。
3. Phase 38：遗物永久货币与结算明细。
4. Phase 39：商店、仓库、配装和破产应急手枪。
5. Phase 40：跨关卡资产返还、再次带入和失败销毁。
6. Phase 41：五关内容、挑战包和数值扩展。
7. Phase 42：完整经济闭环、五关和桌面/移动端回归。
...
最近验证结果：两条命令均为 `95 passed, 0 failed`。
...
4. 只启动 Phase 36：先定义五关与存档结构，不提前接入商店或结算经济。
5. Phase 36 完成后更新计划、交接和测试，再提交推送。

## 13. 建议的新对话开场指令

```text
请读取 D:\codex\ProjectSDC\AGENTS.md、docs\development-handoff.md 和 docs\development-plan.md，检查 Git 状态并运行完整 smoke 与 Phase 35 脚本基线。确认 Phase 35 已推送且工作区干净后，只启动 Phase 36：实现五关数据、4-8 个常规挑战房预算、进度存档和旧存档迁移；不要提前开发遗物货币、商店或跨关卡资产结算。
```

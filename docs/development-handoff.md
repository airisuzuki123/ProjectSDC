...
> 当前状态（2026-07-27）：`master` 将包含本次 Phase 29 信息反馈修正并推送到 `origin/master`。
>
> - Phase 25/26：已完成节奏显示和结算试玩指标，提交 `584f79b`。
> - Phase 27：已按确认改为脚本化路线基线验证，提交 `71af1d2`。
> - Phase 28：Demo 的 5-8 分钟仅保留为结算复盘参考，不设强制 MIA 时限；非 Demo raid 仍为 360 秒。
> - Phase 29：已修正 Demo 自动搜索与首屏帮助提示不一致的问题；不涉及玩法或数值改动。
> - Phase 30：已按用户最终确认完成。Demo 基线包含威胁进度、局内金币、诅咒提示、搜索射击并行与非堆叠背包。
> - 当前进入 Phase 31：挑战框架与入口，暂不开发局外成长模块。
...
- Phase 25：5-8 分钟节奏评估参考。已实现，尚未提交。
...
1. 当前版本冻结新机制。
2. 每轮试玩按 10 局执行。
3. 普通撤离时长以 3-5 分钟为复盘参考，不作为强制限时。
4. 完美撤离时长以 5-8 分钟为复盘参考，不作为强制限时。
5. 最终验收同时覆盖桌面键鼠和移动触控。
...
- 自动逻辑验证不代表 3-5 分钟和 5-8 分钟时长参考已经被体验验收认可。
